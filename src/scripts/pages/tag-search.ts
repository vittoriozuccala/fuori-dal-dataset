...
export class TagSearchController {
	...
	constructor(private readonly root: Document, private readonly mode: SearchMode) {
		...
		if (!input || !list) throw new Error('Struttura di ricerca tag incompleta');
		this.input = input;
		this.list = list;
	...
	private handleInput() {
		...
		this.articleStatus && (this.articleStatus.textContent = 'Ricerca articoli…');
	...
	private updateArticleState(count: number, isSearch: boolean) {
		if (this.postsCount) this.postsCount.textContent = isSearch ? `Trovati ${count} articoli` : `Totale: ${count}`;
		if (this.articleStatus) this.articleStatus.textContent = isSearch ? `Ordinati per pertinenza` : '';
		if (this.emptyState) this.emptyState.style.display = count === 0 ? 'flex' : 'none';
		if (this.emptyMessage) this.emptyMessage.textContent = isSearch ? 'Nessun articolo trovato' : 'Nessun articolo corrispondente';
	}

	private renderSearchError(error: unknown) {
		this.renderInitialCards();
		const message = error instanceof Error ? error.message : 'Ricerca full-text non disponibile';
		if (this.articleStatus) this.articleStatus.textContent = `${message}, riprova`;
	}
	...
	private renderTagMatches() {
		...
		if (this.tagStatus) this.tagStatus.textContent = query ? (visibleCount ? `${visibleCount} tag corrispondenti` : 'Nessun tag corrispondente') : '';
	}
	...
	private syncActiveTagUI() {
		...
		if (this.activeTagTitle) this.activeTagTitle.textContent = this.activeTag ? `# ${this.activeTag}` : 'Tutti gli articoli';
	}
}
