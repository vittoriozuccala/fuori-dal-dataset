...
export class SearchView {
	...
	constructor(root: HTMLElement) {
		...
		if (!input || !results || !liveRegion) throw new Error('Struttura di ricerca incompleta');
		this.input = input;
	...
	render(state: SearchState, documentCount: number, onRetry: RetryHandler) {
		if (state.status === 'idle' || (state.status === 'ready' && !state.query)) {
			...
			this.announce('Digita per cercare');
			return;
		}
		if (state.status === 'loading') {
			this.renderTemplate('loading');
			this.announce('Caricamento indice di ricerca');
			return;
		}
		if (state.status === 'error') {
			...
			this.announce(`${state.message}, puoi riprovare`);
			return;
		}
		if (documentCount === 0) {
			this.renderTemplate('empty-index');
			this.announce('Nessun articolo ricercabile');
			return;
		}
		if (state.hits.length === 0) {
			this.renderTemplate('no-results');
			this.announce('Nessun articolo trovato');
			return;
		}

		this.results.replaceChildren(...state.hits.map((hit, index) => this.createResult(hit, index)));
		this.announce(`Trovati ${state.hits.length} articoli`);
	}
	...
	private renderTemplate(name: string) {
		...
		if (!template) throw new Error(`Manca il template di stato di ricerca: ${name}`);
