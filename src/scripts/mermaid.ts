...
// Area utile del corpo testo in stampa A4 verticale (px CSS).
...
function createDiagram(pre: HTMLPreElement, index: number) {
	...
	header.innerHTML = '<span class="mermaid-diagram__status" aria-live="polite">Rendering…</span>';
...
async function renderFigure(mermaid: MermaidApi, figure: HTMLElement, generation: number) {
	...
	try {
		...
		status.textContent = 'Errore di sintassi del diagramma';
